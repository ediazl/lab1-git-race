...
class HelloController {
    ...
    private val weather_api_key: String? = null
    private var weather_API = "http://api.openweathermap.org/data/2.5/forecast?id=3104324&appid="
    private val weather_options = "&lang=es&units=metric"
    ...
    fun rollTheDice(model: MutableMap<String?, Any?>): String {
        ...
        //OpenWeather API
        if (weather_api_key == "NO_KEY") {
            println("Give a valid API key for the weather information")
        } else {
            weather_API = weather_API + weather_api_key + weather_options
            val restTemplate = RestTemplate()
            val response = restTemplate.getForEntity(weather_API, String::class.java)
            if (response.statusCode == HttpStatus.OK) {
                val mapper = ObjectMapper()
                val actualObj = mapper.readTree(response.body)
                val temp = actualObj.path("list")[1].path("main").path("temp")
                model["temperature"] = temp.asText()
            } else {
                model["temperature"] = "APIDoesNotRespond"
            }
        }
        return "wellcome"
    }
